...
public class IdentityRepositoryImpl extends PropertyReader implements IdentityRepository
{
    ...
    public void addPatient(PatientCreds patient)
    {
        final String patientTemplate = """
                {"id": "%s","name": "%s","email": "%s","groups": ["SHR User","MCI User","Patient"],"profiles": [{"name": "%s","id": "%s","catchment": null}]}""";
        String patientData = String.format(patientTemplate, PATIENT_PORTAL_CLIENT_ID, patient.getName(), patient.getHealthId(), patient.getName(), patient.getHealthId());
